...
import { getRealestate, getFavoritedBuyers } from '../../../actions/realestate/Actions';
import { I18n } from 'react-redux-i18n';
import moment from 'moment'

class RealestateFavorites extends Component {
    ...
    componentDidMount() {
        ...
        if (!code) {
            ...
            this.props.getFavoritedBuyers(code);
        }
    }
                    ...
                    <Segment attached padded='very'>
                        {
                            this.props.realestateCreated ?
                                <Table sortable striped selectable>
                                    ...
                                    <Table.Body>
                                        {
                                            this.props.favoritedBuyers ?
                                                this.props.favoritedBuyers
                                                    .map(buyerFovorited => {
                                                        return (
                                                            <Table.Row key={buyerFovorited.buyer.code}>
                                                                <Table.Cell>
                                                                    {moment.utc(buyerFovorited.createdDate).format('YYYY-MM-DD HH:mm')}
                                                                </Table.Cell>
                                                                <Table.Cell>{buyerFovorited.buyer.identityNumber}</Table.Cell>
                                                                <Table.Cell>{buyerFovorited.buyer.name}</Table.Cell>
                                                                <Table.Cell>{buyerFovorited.buyer.surname}</Table.Cell>
                                                                <Table.Cell>{buyerFovorited.buyer.emailAddress}</Table.Cell>
                                                                <Table.Cell>{buyerFovorited.buyer.phone}</Table.Cell>
                                                                <Table.Cell>{buyerFovorited.buyer.userStatus ? I18n.t("label.USER_STATUS_" + buyerFovorited.buyer.userStatus.code) : ''}</Table.Cell>
                                                            </Table.Row>
                                                        );
                                                    })
                                                : null
...
const mapStateToProps = state => {
    ...
    const { realestateCreated, favoritedBuyers } = state.realestateReducer;
    return {
        lastResponseId,
        user,
        realestateCreated,
...
const hoc = connect(mapStateToProps, mapDispatchToProps)(RealestateFavorites);

export { hoc as RealestateFavorites };
